...
const SubHero = () => {
  ...
  // Build countUp hooks for each stat - use individual hooks
  const stat0 = subHeroConfig.stats[0];
  const stat1 = subHeroConfig.stats[1];
  const stat2 = subHeroConfig.stats[2];
  const stat3 = subHeroConfig.stats[3];
  const count0 = useCountUp(stat0?.value ?? 0, 2000, statsVisible);
  const count1 = useCountUp(stat1?.value ?? 0, 2000, statsVisible);
  const count2 = useCountUp(stat2?.value ?? 0, 2000, statsVisible);
  const count3 = useCountUp(stat3?.value ?? 0, 2000, statsVisible);
  const statCounters = [count0, count1, count2, count3];
