...
function getSpreadsheetId_(overrideId) {
  ...
  // Fallback: container-bound script → active spreadsheet
  try {
  ...
  throw new Error(
    ...
    "해결 방법:\n" +
    "  1) 시트에 바인딩된 스크립트: 에디터에서 initializeApp() 실행\n" +
    "  2) 독립 스크립트: setupSpreadsheetIdFromManual('스프레드시트ID') 실행"
  );
}
...
function assertAdminCredential_(cred) {
  ...
  if (isAdminSessionToken_(raw)) {
    ...
    try {
      CacheService.getScriptCache().put(cacheKey, cacheVal || "1", ADMIN_SESSION_TTL_SEC_);
    } catch (e2) {}
...
function clearAdminSession_(cred) {
  var raw = String(cred || "").trim();
  if (!raw || !isAdminSessionToken_(raw)) return false;
  try {
    CacheService.getScriptCache().remove(getAdminSessionCacheKey_(raw));
  } catch (e) {}
  return true;
}
